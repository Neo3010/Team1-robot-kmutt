...
- **ESP32-WROVER board** — connects to WiFi and streams live MJPEG video over HTTP to the browser.
...
[Browser]
   ...
   │
   └── HTTP WiFi stream ◄──────────── [ESP32-WROVER Board]
                                           └── OV2640 Camera
```
...
├── cameraesp32/
│   └── cameraesp32.ino             ← Flash to ESP32-WROVER (Espressif)
...
| Microcontroller (Camera) | **ESP32-WROVER (Espressif)** | Has 8MB PSRAM built into the module — required for 3-buffer camera streaming. The pin block in the code matches the WROVER pinout. Other models need their own pin definitions. |
...
The ESP32-WROVER has **8MB of PSRAM built into the module**. The code detects this automatically with `psramFound()` and enables 3 frame buffers for smooth streaming. If you switch to a board without PSRAM, it falls back to 1 frame buffer and lower quality automatically — no code change needed.

---

#### 🟠 Camera Pin Definitions

The full pin block at the top of the file is pre-configured for the **ESP32-WROVER (Espressif)**. Do not change these unless you are using a different board layout.

```cpp
// Only change if using a NON ESP32-WROVER (Espressif) board:
...
If you use a different board model, replace the pin block at the top of the file with the correct definitions for your board. Espressif's `CameraWebServer` example in Arduino IDE contains ready-made pin blocks for all common ESP32 camera modules.
...
The interface auto-connects to this IP on page load. Set it to your ESP32-WROVER's IP (printed in Serial Monitor after the camera boots).
...
| ESP32-WROVER Pin | Connection |
|-----------------|------------|
| 5V | 5V supply |
| GND | Ground |

> ✅ The ESP32-WROVER has a native USB port — no USB-to-TTL adapter or GPIO0 trick required.
...
### Flash the ESP32-WROVER

The ESP32-WROVER has a native USB port — just plug it in directly. No USB-to-TTL adapter or GPIO0 wiring needed.

1. Open `cameraesp32/cameraesp32.ino`
...
> ⚠️ If upload fails, hold the **BOOT** button on the board while clicking Upload, then release once upload starts. This manually enters flash mode.
...
1. Enter your ESP32-WROVER's IP in the bar at the bottom → **▶ STREAM**
